...
| The type your chain has after each step, and that the value at the end is that type | [`test_typing.py`](https://github.com/Solganis/assertpy2/blob/main/tests/test_typing.py), 362 `assert_type` checks under ty, mypy `--strict`, Pyright and Pyrefly, zero suppressions |
