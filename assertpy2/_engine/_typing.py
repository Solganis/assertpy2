...
if TYPE_CHECKING:
    import datetime
    from collections.abc import Awaitable, Callable, Hashable, Iterable, Mapping, Sized
    ...
    class _DateAssertion(_CoreAssertion, Protocol):
        ...
        def is_not_between(self, low: datetime.date, high: datetime.date) -> Self: ...
    ...
    class _DateTimeAssertion(_CoreAssertion, Protocol):
        ...
        def is_not_between(self, low: datetime.datetime, high: datetime.datetime) -> Self: ...
    ...
    class _InvokedAssertion(_TextAssertion, Protocol):
        ...
        # a `list` is a subgroup, so the entry type is recursive; spelled with `Any` inside because a
        # recursive alias declared here is what ty reads as `Any` anyway, measured in `_typing.py`'s own
