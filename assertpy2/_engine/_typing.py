...
if TYPE_CHECKING:
    ...
    # Every predicate parameter below reads `Callable[[Any], object]`, or `[Any, Any]` where the runtime
    # hands over a pair.  Both halves of that are measured rather than assumed.
    #
    # The verdict is `object` because the runtime reads it for truth: a predicate returning `1`, a plain
    # object or `numpy.bool_` keeps the item, and one returning `None` drops it.  `bool` was tried here
    # and refused a `numpy.bool_` verdict, which is the mistake this project already made once with the
    # numeric bound, where a list of types rejected `numpy.int64` and broke working suites.
    #
    # Keeping `bool` as a lint against a forgotten comparison (`lambda row: row.name` instead of
    # `row.name == "a"`) was the earlier reasoning, and it does not hold: such a predicate is not
    # vacuously true, an empty string filters its row out, and `bool` catches nothing when the field is
    # `Any` or already `bool`, which is the common case.
    #
    # The arity is the runtime's too, measured in both directions: `each` and its neighbours call the
    # predicate with one argument, so a two-parameter function raises `missing 1 required positional
    # argument`, while `zip_satisfies` and `comparators` call with two and a one-parameter function
    # raises `takes 1 positional argument but 2 were given`.
    ...
    class _StructureAssertion(Protocol):
        ...
        def extracting(
            self,
            # a selector stays `object`, and that is a conclusion rather than laziness.  Whether one
            # works depends on the row, which is not knowable here, and every narrowing tried refused
            # code that runs:
            #   `str | int`           refuses a tuple, a float, a byte string and `None`, all real keys
            #   `Hashable`            refuses `slice(0, 2)` on CPython 3.10, where slices are unhashable
            #                         and `extracting(slice(0, 2))` still returns `[(1, 2)]`
            #   `Hashable | slice`    refuses a list key on a mapping row whose `__getitem__` takes one,
            #                         measured: it returns `['got []']` rather than raising
            # The runtime says no in its own words when a selector does not fit the row it met
            # the first selector is required, because the runtime refuses a call without one
            # (`ValueError: one or more name args must be given`) and a type can say that outright
            name: object,
            *names: object,
            ...
            # each callable is handed one item: a two-parameter one raises `missing 1 required
            # positional argument` when the runtime calls it with one, so the arity is contract.
            #
            # any mapping, not only a `dict`: measured on `MappingProxyType` and on a class that
            # implements the interface and nothing else, both accepted.  `Mapping` also keeps a
            # `dict[str, str]` variable usable, which the invariant `dict[str, object]` refused.
            #
            # `str` keys are a stated limit rather than the whole runtime behaviour: a mapping with
            # other keys is accepted and those keys are then skipped, so a wider type would describe a
            # filter that silently does nothing
            filter: str | Mapping[str, object] | Callable[[Any], object] = ...,  # noqa: A002  # public keyword
            # `sort` is narrower than a selector, and that shape is the runtime's own rule: a single
            # index is refused (`given sort arg must be a str, an iterable, or a callable`) while the
            # same index inside a list is honoured.  Its members stay `object` for the reason above,
            # measured on `sort=[slice(0, 2)]`, which sorts tuple rows and is unhashable on 3.10
        ...
        def each(self, matcher: Matcher | Callable[[Any], object], *, allow_empty: bool = ...) -> Self: ...
        def all_satisfy(self, matcher: Matcher | Callable[[Any], object], *, allow_empty: bool = ...) -> Self: ...
    ...
    class _CoreAssertion(Protocol):
        ...
        @overload
        def satisfies(self, matcher: Matcher | Callable[[Any], object]) -> Self: ...
        def all_fields_satisfy(
            self, matcher: Matcher | Callable[[Any], object], *, allow_empty: bool = ...
        ) -> Self: ...
        ...
        def snapshot(
            ...
            *,
            ...
            tolerance: float | None = ...,
            comparators: dict[object, Callable[[Any, Any], object]] | None = ...,
            placeholders: dict[Hashable, Matcher | Callable[[Any], object]] | None = ...,
        ) -> Self: ...
    ...
    class _TextAssertion(_MembershipAssertion, _RepeatableAssertion[str], _SizedAssertion, _CoreAssertion, Protocol):
        ...
        def filtered_on(self, predicate: Matcher | Callable[[str], object]) -> _ListAssertion[str]: ...
    ...
    class _IterableAssertion(
        ...
        def each(self, matcher: Matcher | Callable[[_E], object], *, allow_empty: bool = ...) -> Self: ...
        def all_satisfy(self, matcher: Matcher | Callable[[_E], object], *, allow_empty: bool = ...) -> Self: ...
        def any_satisfy(self, matcher: Matcher | Callable[[_E], object]) -> Self: ...
        def none_satisfy(self, matcher: Matcher | Callable[[_E], object]) -> Self: ...
        def satisfies_exactly(self, *matchers: Matcher | Callable[[_E], object]) -> Self: ...
        def satisfies_exactly_in_any_order(self, *matchers: Matcher | Callable[[_E], object]) -> Self: ...
        def zip_satisfies(
        ...
        def filtered_on(self, predicate: Matcher | Callable[[_E], object]) -> _ListAssertion[_E]: ...
    ...
    class _DictAssertion(_MembershipAssertion, _StructureAssertion, _SizedAssertion, _CoreAssertion, Protocol[_K, _V]):
        ...
        def each(self, matcher: Matcher | Callable[[_K], object], *, allow_empty: bool = ...) -> Self: ...
        def all_satisfy(self, matcher: Matcher | Callable[[_K], object], *, allow_empty: bool = ...) -> Self: ...
        ...
        def filtered_on(self, predicate: Matcher | Callable[[_K], object]) -> _ListAssertion[_K]: ...
    ...
    class _BytesAssertion(_MembershipAssertion, _SizedAssertion, _CoreAssertion, Protocol[_B_co]):
        ...
        def filtered_on(self, predicate: Matcher | Callable[[int], object]) -> _ListAssertion[int]: ...
