...
CASES = [
    Case("dict", {"id": 1, "name": "a"}, lambda b: b.is_equal_to({"id": 2, "name": "b"})),
    Case("nested dict", {"a": {"b": 1}}, lambda b: b.is_equal_to({"a": {"b": 2}})),
    Case("list", [1, 2, 3], lambda b: b.is_equal_to([1, 9, 3])),
    Case("set", {1, 2}, lambda b: b.is_equal_to({1, 9})),
    Case("tuple", (1, 2), lambda b: b.is_equal_to((1, 9))),
    Case("scalar", 1, lambda b: b.is_equal_to(2)),
    Case("none against a value", None, lambda b: b.is_equal_to(1)),
    Case("short line", "hello", lambda b: b.is_equal_to("hallo")),
    Case("long line", "x" * 90 + "A" + "y" * 90, lambda b: b.is_equal_to("x" * 90 + "B" + "y" * 90)),
    Case("many lines", "a\nb", lambda b: b.is_equal_to("a\nc")),
    Case("bytes", b"abd", lambda b: b.is_equal_to(b"abc")),
    Case("membership", [1, 2, 3], lambda b: b.contains(9)),
    Case("length", "foo", lambda b: b.is_length(4)),
    Case("negated", 1, lambda b: b.not_.is_equal_to(1)),
    Case("described", 1, lambda b: b.described_as("label").is_equal_to(2)),
    Case("one cause", {"a": b"x", "b": b"y"}, lambda b: b.is_equal_to({"a": "x", "b": "y"})),
    Case("mixed causes", {"a": b"x", "b": 1}, lambda b: b.is_equal_to({"a": "x", "b": 2})),
]
