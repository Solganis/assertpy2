...
- [**HTTP responses**](https://solganis.github.io/assertpy2/recipes/#test-an-http-api-response): assert on the response itself and every failure names the request it came from, with `decoded_as_json()` to step into the body. Duck-typed, so httpx, httpx2, requests, Django and the Starlette, FastAPI and Flask test clients all work and none is a dependency.
